...
function processWatermarkRoi(ctx, detection, options) {
    ...
    // Get the ROI before processing
    const roi = ctx.getImageData(position.x, position.y, position.width, position.height);

    // CRITICAL FIX: Always use the seed alpha gain from the global detection
    // This ensures consistent watermark removal across all frames, preventing blinking
    // The seed gain is calculated during detection from multiple sample frames and
    // represents the optimal alpha value for the entire video
    const alphaGain = Number.isFinite(options.seedAlphaGain) && options.seedAlphaGain > 0
        ? options.seedAlphaGain
        : DEFAULT_ALPHA_GAIN;

    // CRITICAL FIX: Apply watermark removal to ALL frames consistently
    // Do NOT skip frames based on per-frame scoring - this causes blinking
    // The detection phase already validated the watermark presence
    const processed = applyRoiRemoval(roi, alphaMap, alphaGain);
    ctx.putImageData(processed, position.x, position.y);

    // DISABLE temporal stabilization and cleanup - they cause blinking artifacts
    // The temporal stabilization attempts to blend frames but this creates
    // inconsistent watermark removal when the alpha changes frame-to-frame
    // Apply cleanup after removal - but skip temporal methods that cause blinking
    ...
    // Skip all temporal stabilization - it causes the blinking effect
    return {
        alphaGain,
        frameScore: null,
        skipped: false,
...
async function processWatermarkRoiAsync(ctx, detection, options) {
    ...
    // Get the ROI before any processing
    const roi = ctx.getImageData(position.x, position.y, position.width, position.height);
    ...
    // Apply async cleanup - but disable temporal reuse which can cause blinking
    const cleanupResult = await applyVideoResidualCleanupAsync(ctx, position, alphaMap, {
        ...
        allenkFdncnnTemporalReuse: false, // Disable temporal reuse to prevent blinking
...
export async function processVideoWatermarkFrame(ctx, detection, trackStates = new Map(), options = {}) {
    // CRITICAL FIX: DO NOT re-select detection per frame - this causes blinking!
    // Use the original detection from the detection phase consistently across all frames
    const selectedDetection = detection;

    // CRITICAL FIX: Use the seed alpha gain from the global detection consistently
    // This prevents blinking caused by per-frame alpha gain variations
    const consistentAlphaGain = selectedDetection?.alphaSeed?.seedGain ?? options.seedAlphaGain ?? DEFAULT_ALPHA_GAIN;

    // Process frame WITHOUT temporal state to ensure consistency
    const frameResult = await processWatermarkRoiAsync(ctx, selectedDetection, {
        ...options,
        seedAlphaGain: consistentAlphaGain,
        // Disable all temporal features that can cause blinking
...
export async function removeGeminiVideoWatermark(file, options = {}) {
    ...
    if (!canEncodeAvc) {
        console.warn('[gwr-video] canEncodeVideo(avc) returned false — attempting anyway, browser may still support it.');
    }
        ...
        for await (const sample of sink.samples()) {
            ...
            try {
                ...
                const frameResult = await processVideoWatermarkFrame(ctx, detection, trackStates, {
                    ...
                    adaptiveAlpha: false, // Disable adaptive alpha to prevent per-frame variations
                    residualCleanupStrength,
                    highQualityCleanup,
                    denoiseBackend: 'none', // Force disable denoising to prevent blinking
                    ...
                    allenkFdncnnSigma: options.allenkFdncnnSigma,
                    allenkFdncnnPadding,
                    allenkFdncnnTemporalReuse: false, // Disable temporal reuse
